...
for x in range(limit+1):
    for y in range(limit-x+1):
        for w in range(limit-x-y+1):
            z = limit-x-y-w
            calories = (x*ingredients[0][-1] + y*ingredients[1][-1]
                      + w*ingredients[2][-1] + z*ingredients[3][-1])
            if calories == 500:
                score = (max(x*ingredients[0][0] + y*ingredients[1][0]
                ...
                if score > high_score:
                    high_score = score
                    # print(x, y, w, z, calories, high_score)

print("part2:", high_score)
