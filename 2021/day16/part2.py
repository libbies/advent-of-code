...
class Packet():
    ...
    def __init__(self, bits, hexits=None):
        ...
        if self.type==0:   # op:add
            ...
            self.value = reduce(int.__mul__, (p.value for p in self.subpackets))
            ...
            self.value = int("0b"+''.join(b[-4:] for b in self._groups), 2)
        elif self.type==5: # op:gt
            assert len(self.subpackets)==2
            self.value = self.subpackets[0].value>self.subpackets[-1].value
...
answer = Packet(bits=None, hexits=open("input.txt").read().strip()).value

print("aoc 2021 day 16 part 2:", answer)
