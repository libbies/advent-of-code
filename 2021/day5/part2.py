...
for p1, _, p2 in inputs:
    ...
    if y1==y2:
        for point in ((x, y1) for x in range(min(x1, x2), max(x1, x2)+1)):
            points[point] += 1
    if x1>x2 and y1>y2:
        for point in zip(range(x1, x2-1, -1), range(y1, y2-1, -1)):
            points[point] += 1
    if x1<x2 and y1<y2:
        for point in zip(range(x1, x2+1), range(y1, y2+1)):
            points[point] += 1
    if x1>x2 and y1<y2:
        for point in zip(range(x1, x2-1, -1), range(y1, y2+1)):
            points[point] += 1
    if x1<x2 and y1>y2:
        for point in zip(range(x1, x2+1), range(y1, y2-1, -1)):
            points[point] += 1
